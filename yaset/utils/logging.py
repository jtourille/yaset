...
class TrainLogger:
    ...
    def add_histogram(self,
        ...
        self.writer.add_histogram(name, value, global_step=global_step, bins=bins)
    ...
    def get_last_checkpoint_string(self,
                                   idx_iteration: int = None):
        ...
        for k, v in self.checkpoints[idx_iteration][last_checkpoint_id].items():
            if type(v) is int:
    ...
    def dump_to_disk(self,
                     custom_log_file: str = None,
                     tensorboard_log_file: str = None):
        ...
        with open(os.path.abspath(custom_log_file), "w", encoding="UTF-8") as output_file:
    ...
    def load_json_file(self,
        ...
        with open(os.path.abspath(filepath), "r", encoding="UTF-8") as input_file:
            payload = json.load(input_file)

        self.losses = payload["losses"]
        self.dev_scores = {int(k): v for k, v in payload["dev_scores"].items()}
        self.dev_other_scores = {int(k): v for k, v in payload["dev_other_scores"].items()}
