...
def extract_mappings_and_pretrained_matrix(
    ...
    if options.get("embeddings").get("pretrained").get("use"):
        ...
        if pretrained_configuration.get("format") == "gensim":
            model_filepath = os.path.abspath(
                pretrained_configuration.get("model_path")
            )
            ...
            all_mappings["tokens"] = [
                pad_symbol,
                unk_symbol,
            ] + embedding_obj.wv.index2word
            all_mappings["tokens"] = {
                v: k for k, v in enumerate(all_mappings["tokens"])
            }
            ...
            for _ in range(2):
                ...
                pretrained_matrix = np.append(
                    pretrained_matrix, vector, axis=0
                )

        elif pretrained_configuration.get("format") == "glove":
            source_glove_filepath = os.path.abspath(
                pretrained_configuration.get("model_path")
            )
            target_w2v_filepath = os.path.join(
                os.path.abspath(output_dir), "model.w2v"
            )
    ...
    ner_labels = extract_ner_labels(
        instance_file=options.get("data").get("train_file")
    )
    all_mappings["ner_labels"] = {
        v: k for k, v in enumerate(sorted(ner_labels))
    }

    char_list = extract_char_list(
        instance_file=options.get("data").get("train_file")
    )
    ...
    all_mappings["characters_literal"] = {
        k: i for i, k in enumerate(char_list)
    }
